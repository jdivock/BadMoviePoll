...
class MovieService {
    ...
    search(query) {
        ...
        return new Promise(function(resolve, reject) {
            ...
            self.reject = reject;
	    var url = self.rottenTomatoesUrl + 'movies.json?q=' + query + '&page_limit=10&page=1&apikey=qd9psweg5mupm8wqdct622bk';

            return jsonp(
                url , (err, data) => {
                    err ? reject(err) : resolve(data); // jshint ignore:line
                }
            );
        }).then(function(resp) {
    ...
    setMovieDate(movieKey, movieDate, profile){
